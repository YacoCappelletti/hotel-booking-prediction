"""Phase 4 tests: artifacts load, prediction shape, documented test minimums.

Test metrics are read from docs/json/model_performance.json (the single G6
evaluation); this suite never re-evaluates the test set.
"""
...
def test_metadata_complete():
    ...
    assert METADATA["test_metrics"] is not None, "test evaluation missing (G6)"


def test_prediction_shape_and_range(artifacts):
...
def test_explainer_produces_factors(artifacts):
